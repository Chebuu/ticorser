...
ApplyNOISeq <- function(counts.dataframe, design.dataframe, factors.column=NULL,
                        ...
                        test.type=c("NOISeqBio", "NOISeq"))
{ ## DA PERFEZIONARE CON ULTERIORI ARGOMENTI
    ...
    if(test.type == "NOISeqBio") {
        ...
        de.results <- noiseqbio(input=noiseq.dataset, norm="n",
                                        ...
                                        n.genes.per.break=1)
{
  ...
  data.counts.mean.filtered <- data.counts.mean[which(data.counts.mean <=
                                        summary(data.counts.mean)["3rd Qu."])]
...
ConvertGeneNamesToEnsembl <- function(map.dataframe, named.dataset) {
    ...
    datanames <- data.frame(rownames= rownames(named.dataset), ensembl_ids=NA,
                        gene_names=NA, ind_map=NA, ens_present=FALSE)

    ind.genes.to.map <- which(tolower(datanames$rownames) %in%
                            tolower(map.dataframe$external_gene_name))
    dataset.ens <- named.dataset
    i <- 0
    for (ind in ind.genes.to.map) {
    gene.name <- rownames(dataset.ens)[ind]

    ind.map <- which(tolower(map.dataframe$external_gene_name) ==
                        tolower(gene.name))
    ...
    if(length(ind.map) == 0)
    {
        ...
        ind.check <- which( tolower(rownames(dataset.ens)) ==
                            tolower(map.dataframe$ensembl_gene_id[ind.map]))
      ...
      datanames[ind, "ensembl_ids"] <-
                    paste(map.dataframe$ensembl_gene_id[ind.map], collapse="; ")
    ...
    write.table(datanames, file="./rownames_map.txt", sep="\t", row.names=FALSE,
            quote=FALSE)
}
...
#' @param output.folder
#' @param save.flag
#' @param prefix.output.file
#' @param gtf.attr.type
#' @param n.thread
#' @param strand.specific
#' @param is.paired.end
#'
#' @return
#' @export
#'
#' @examples
countBamFilesFeatureCounts <- function(bam.files, annotation.file,
                                        output.folder=NULL,
                                        save.flag=FALSE,
                                        prefix.output.file,
                                        ...
                                        is.paired.end=FALSE)
{
    if(is.null(annotation.file)) stop("Please provide an annotation file!")
    ...
    if(save.flag)
    {
        fc.count.file.name <- paste0(prefix.output.file,
                                    "_Counts_FeatureCounts.txt")
        ...
        WriteDataFrameAsTsv(data.frame.to.save=fc_SE[1]$counts,
                            file.name.path=fc.count.file.path)
    }
